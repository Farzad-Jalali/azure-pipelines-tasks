...
describe('AzureCLI Suite', function () {
    ...
    function addInlineObjectJson(responseFileName, nameOfFileToBeCreated) {
        var jsonFileObject = JSON.parse(fs.readFileSync(path.join(__dirname, responseFileName)).toString());
    ...
    function deleteInlineObjectJson(responseFileName, nameOfFileToBeCreated) {
        var jsonFileObject = JSON.parse(fs.readFileSync(path.join(__dirname, responseFileName)).toString());
        if (os.type() === "Windows_NT") {
    ...
    var publishsettingFileName = '.*subscriptions.*';
    var inlineScriptName = '.*azureclitaskscript.*';
    ...
    it('successfully login azure RM and run shell script (inline)', (done) => {
        var responseFileName = 'azureclitaskPass.json';
