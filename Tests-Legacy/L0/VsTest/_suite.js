...
const psm = require("../../lib/psRunner");
const path = require("path");
const os = require("os");
const mockHelper = require("../../lib/mockHelper");
const fs = require("fs");
...
describe('VsTest Suite', function () {
    ...
    it('Vstest task with results directory as absolute path in run settings file', (done) => {
        ...
        const responseJsonFilePath = path.join(__dirname, 'vstestGood.json');
        ...
        const newResponseFilePath = path.join(__dirname, 'newresponse.json');
    ...
    it('Vstest should throw proper error for invalid vstest.console.exe location', (done) => {
        ...
        tr.run()
            ...
            assert(tr.resultWasSet, 'task should have set a result');
            assert(tr.failed, 'task should have failed');
            assert(tr.stdout.indexOf('The location of \'vstest.console.exe\' specified \'C:/vstest.console.exe\' does not exist.') >= 0, 'should throw invalid path error');
            done();
        })
            .fail((err) => {
            console.log(tr.stdout);
