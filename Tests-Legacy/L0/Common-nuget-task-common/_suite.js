        ...
        for (const expected of expectedQuirksByVersion) {
            it(`Has the expected quirks for NuGet ${expected.displayVersion} (${expected.version})`, function () {
                ...
                for (const quirkInExpectedSet of expected.quirks) {
                    assert(actualQuirks.has(quirkInExpectedSet), `version should report quirk ${NuGetQuirkName[quirkInExpectedSet]}, but does not`);
                }
                for (const quirkInActualSet of actualQuirks) {
                    assert(expected.quirks.has(quirkInActualSet), `version unexpectedly reports quirk ${NuGetQuirkName[quirkInActualSet]}`);
                }
            });
        }
    });
